...
class Sheets {
   ...
   */
  async batchUpdate(spreadsheetId, rows) {
    return new Promise((resolve, reject) => {
   ...
   * Removes rows
   * @param spreadsheetId
   * @param rows Array of row numbers to remove. For example: [1, 2, 5] removes rows 5, 2 and 1
   * @returns {Promise<Promise<unknown>>}
   */
  // TODO: combine neighbour row numbers
  async batchDelete(spreadsheetId, rows) {
    return new Promise((resolve, reject) => {
      // [START sheets_batch_update]
      let requests = [];
      rows.sort((a, b) => b - a);
      ...
      this.sheetsService.spreadsheets.batchUpdate({
        ...
        if (err) {
          ...
          // const findReplaceResponse = response.replies[1].findReplace;
          // console.log(`${findReplaceResponse.occurrencesChanged} replacements made.`);
          console.log(`${response.data.replies.length} rows deleted.`);
        }
        // [START_EXCLUDE silent]
        resolve(response);
        // [END_EXCLUDE]
   ...
   * @return {Promise<ValueUpdateResponse>} The updated values response.
   */
  async batchUpdateValues(spreadsheetId, data_) {
    return new Promise((resolve, reject) => {
      // [START sheets_batch_update_values]
      let data = [];
      for (const row_ of data_) {
        data.push({
          majorDimension: 'ROWS',
          range: 'A' + row_.row,
          values: [row_.values],
